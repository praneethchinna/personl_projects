...
final notificationsNotifierProvider = StateNotifierProvider.autoDispose<NotificationsNotifier, AsyncValue<NotificationResponse>>((ref) {
  final repository = ref.read(notificationsRepositoryProvider);
  final userId = ref.read(userProvider).userId ?? 0;
  return NotificationsNotifier(repository, userId);
});

class NotificationsNotifier extends StateNotifier<AsyncValue<NotificationResponse>> {
  ...
  NotificationsNotifier(this._repository, this._userId) : super(const AsyncValue.loading()) {
  ...
  Future<void> loadInitialNotifications() async {
    try {
      ...
      final response = await _repository.getNotifications(_userId, page: 1, pageSize: _pageSize);
  ...
  Future<void> loadMoreNotifications() async {
    ...
    try {
      ...
      final response = await _repository.getNotifications(_userId, page: nextPage, pageSize: _pageSize);
