...
class _ForgotOtpScreenState extends ConsumerState<ForgotOtpScreen> {
  ...
  void verifyOtp() {
    ...
    ref
        .read(repoProvider)
        .verifyForgotOtp(numberController.text, otpController.text)
        .then((value) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      ...
      Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => ResetPassword(numberController.text),
          ));
  ...
  void getTop() {
    EasyLoading.show();
    ref.read(repoProvider).getForgotOtp(numberController.text).then((value) {
      setState(() {
        isOtpSent = true;
      });
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
